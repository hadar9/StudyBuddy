    ...
    for (var i in groups) {
      if (groups[i].group_name === '') {
        if (user_id == groups[i].names[0]) {
          other_users.push([
            groups[i].group[1]._id,
            groups[i].names[1],
            groups[i]._id,
            groups[i].group[1].avatar,
          ]);
        } else {
          other_users.push([
            groups[i].group[0]._id,
            groups[i].names[0],
            groups[i]._id,
            groups[i].group[0].avatar,
          ]);
        }
      } else {
        other_users.push(['', groups[i].group_name, groups[i]._id, null]);
      }
    }
